...
LEVELDB_VERSION = str(0).encode('utf-8')
...
class LevelDB(BaseDB):
    ...
    def __init__(
        ...
        # LEVELDB_VERSION is not part of the dict because it does not start with the delimiter
        v = self.db.get(b'LEVELDB_VERSION')
        if v is None:
            self.db.put(b'LEVELDB_VERSION', LEVELDB_VERSION)
        else:
            assert v == LEVELDB_VERSION
        self._debug()
